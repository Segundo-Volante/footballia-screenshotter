...
On first launch, you'll see the setup wizard. Enter your team name, season (e.g. 2024-25), and select the competitions you want to track.

![Setup Screen](screenshots/01-setup.png)

### Step 2: Home Dashboard

After setup, the home dashboard shows all available tools. Each card represents a different way to add and process matches. Use **Quick Capture** for fast one-off captures, or browse the **Match Library** to manage your collection.

![Home Dashboard](screenshots/02-home.png)

### Step 3: Add a Match via Quick Capture

Click **Quick Capture** and paste a Footballia match URL. The app will automatically scrape match metadata (teams, date, competition, lineups, goals) from the page.

![Quick Capture](screenshots/03-quick-capture.png)

### Step 4: Configure Capture Settings

Before starting a capture, configure your settings:
- **Classification Task**: Choose what to classify (camera angles, formations, events, etc.)
- **AI Provider**: Select OpenAI GPT-4o-mini, Google Gemini Flash, or Manual mode
- **Screenshot Targets**: Set how many frames you want per category
- **Capture Presets**: Pick a preset (YOLO for maximum coverage, Balanced for normal use, Quick Test for trying things out, or Tactical Analysis for formation-focused captures)

![Capture Configuration](screenshots/04-capture-config.png)

### Step 5: Live Capture Dashboard

Once capture starts, the live dashboard shows real-time progress:
- Current video timestamp and frame being processed
- AI classification results for each captured frame
- Per-category progress bars showing how close you are to your targets
- Running API cost tracker

![Live Capture - Early](screenshots/05-capture-live.png)

As the capture progresses, you can watch the progress bars fill up and see the AI's classifications in real-time:

![Live Capture - Progress](screenshots/06-capture-progress.png)

### Step 6: Discover Matches by Coach or Player

Use the **Browse by Coach/Player** tool to find matches on Footballia. Enter a coach or player page URL to scrape all available matches.

![Navigator](screenshots/07-navigator.png)

The navigator will return a list of all matches found, which you can then add to your library for capture:

![Navigator Results](screenshots/08-navigator-results.png)

### Step 7: Match Library

The **Match Library** shows all matches you've added, with their metadata, capture status, and frame counts. Use this view to manage your match collection and start captures.

![Match Library](screenshots/09-match-library.png)

### Step 8: Statistics & Export

The **Statistics** view shows aggregated data across all your captures. From here you can export your classified frames as datasets in standard ML formats (COCO JSON, ImageNet folder structure, CSV, or HuggingFace Datasets).

![Statistics & Export](screenshots/10-statistics.png)
